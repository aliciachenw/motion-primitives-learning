...
cluster_ind=unique(data(:,clusloc));
cluster_num=length(cluster_ind);
% total=length(data(:,clusloc));
% cluster_fre=zeros(1,cluster_num);
% abnormal=[];
% abnormal_ind=[];
% label=data(:,clusloc)+1;
% for i=1:cluster_num
...
%         data(i,18)=new_cluster_ind(j)-1;
%     else
%         data(i,clusloc)=cluster_num-1;
%     end
% end
% cluster_ind=[];
% cluster_ind=new_cluster_ind-1;
%% n-gram sample
trial_ind=unique(data(:,3));
...
for i=1:trial_num
    ...
    for j=1:step:r-winlen+1
        ...
        for k=1:winlen-num_gram+1
            temp=temp_data(k:k+num_gram-1,clusloc);
            temp=temp';
        ...
        ngram=[temp_data(1,1),temp_data(1,2),temp_data(1,3),temp_data(1,4),temp_data(winlen,5),ngram];
...
outfile='pri_ngram_window_v2.csv';
