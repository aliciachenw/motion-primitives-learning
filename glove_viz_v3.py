...
def load_cal(ID, trial, hand):
    ...
    calpath = 'D:\python_program\HandMotionPrimitive\data\withlabel_v3\ID' + str(
        ID) + '\\feature\\' + hand + '_IMU_' + str(trial) + '_calrotm.csv'
...
def plot_posture(quaternions, ID, trial, hand, label):
    ...
    ax.plot(coord_thumb[0, x_id], coord_thumb[0, y_id], coord_thumb[0, z_id], 'co-', fillstyle='none',
            ms=5, label='thumb')
    ax.plot(coord_index[0, x_id], coord_index[0, y_id], coord_index[0, z_id], 'bo-', fillstyle='none',
            ms=5, label='index')
    ax.plot(coord_middle[0, x_id], coord_middle[0, y_id], coord_middle[0, z_id], 'ro-', fillstyle='none',
            ms=5, label='middle')
    ax.plot(coord_engage[0, x_id], coord_engage[0, y_id], coord_engage[0, z_id], 'go-', fillstyle='none',
            ms=5, label='ring')
    ax.plot(coord_pinky[0, x_id], coord_pinky[0, y_id], coord_pinky[0, z_id], 'yo-', fillstyle='none',
            ms=5, label='pinkie')
    ax.plot(coord_palm[0, x_palm], coord_palm[0, y_palm], coord_palm[0, z_palm], 'ko-', fillstyle='none',
            ms=5, label='palm')
...
def plot_primitive(data, ID, trial, hand, label):
    ...
    if hand == 'L':
        ...
        handpath = 'right'
    # filename = 'D:\python_program\HandMotionPrimitive\Clustering\\results_v4\ID'+str(ID)+'\\'+handpath +'\primitive\primitive_'+str(label)+'.gif'
    #convert_path: 'C:\Program Files\ImageMagick-6.9.0-Q16\convert.exe'
    #FFwriter = an.FFMpegWriter()
    # writer = an.ImageMagickFileWriter()
